...
class PixelUnshuffle:
    ...
    def _process_img(self, results):
        """Pad images according to ``self.size``."""
        for key in results.get('img_fields', ['img']):
            tensor_to_torch = torch.from_numpy(results[key])
            tensor_to_torch = tensor_to_torch.permute(2,0,1)
            processed_img = torch.pixel_unshuffle(tensor_to_torch,downscale_factor=self.scale)
            results[key] = processed_img.permute(1,2,0).data.cpu().numpy()
        results['pad_shape']=( results['pad_shape'][0]//self.scale,results['pad_shape'][1]//self.scale,3)
        results['scale_factor']/=self.scale

    def _process_bboxes(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        for key in results.get('bbox_fields', []):
            bboxes = results[key] /self.scale
            results[key] = bboxes

    def _process_masks(self, results):
        for key in results.get('mask_fields', []):
            results[key] = cv2.resize(results[key],fx=1/self.scale,fy=1/self.scale)

    def _process_seg(self, results):
        for key in results.get('seg_fields', []):
            results[key] = cv2.resize(results[key],fx=1/self.scale,fy=1/self.scale)
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Updated result dict.
        """
        self._process_img(results)
        self._process_bboxes(results)
        self._process_masks(results)
        self._process_seg(results)
        return results
