...
@PIPELINES.register_module()
class LoadImageListFromFile:
    '''Chongwang constructed'''
    def __init__(self,
                 to_float32=False,
                 color_type='color',
                 channel_order='bgr',
    ...
    def __call__(self, results):
        if self.file_client is None:
            self.file_client = mmcv.FileClient(**self.file_client_args)
        for n, filename in enumerate(results['img_info']['filenamelist']):
            if filename is not None:
                ...
                img = np.zeros(shape=(results['img_info']["height"], results['img_info']["width"],3),dtype=np.uint8)
            if self.to_float32:
                img = img.astype(np.float32)
            results['img_%03d'%n] = img
            results['img_fields'].append('img_%03d'%n)
        return results
...
@PIPELINES.register_module()
class LoadAnnotationsDrawSeg(LoadAnnotations):

    def _load_semantic_seg(self, results):
        gt_semantic_seg = np.zeros(shape=results['img'].shape[0:2],dtype=np.uint8)
        for bbox in results['gt_bboxes']:
            bbox = np.array(bbox,dtype=int)
            gt_semantic_seg = cv2.rectangle(gt_semantic_seg, bbox[0:2], bbox[2:4], color=1, thickness=-1)

        results['gt_semantic_seg'] = gt_semantic_seg.astype(np.float32)
        results['seg_fields'].append('gt_semantic_seg')
        return results
