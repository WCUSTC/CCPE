...
class TransformerEncoderPixelDecoder(PixelDecoder):
    ...
    def forward4maskformer(self, feats, input_img_h, input_img_w):
        """
        ChongWang for maskformer
        """
        feat_last = feats[-1]
        bs, c, h, w = feat_last.shape
        padding_mask = feat_last.new_ones((bs, input_img_h, input_img_w), dtype=torch.float32)
        for i in range(bs):
            img_h, img_w, _ = input_img_h, input_img_w,0
            padding_mask[i, :img_h, :img_w] = 0
        padding_mask = F.interpolate(  padding_mask.unsqueeze(1),  size=feat_last.shape[-2:], mode='nearest').to(torch.bool).squeeze(1)
        pos_embed = self.positional_encoding4onnx(padding_mask)
        feat_last = self.encoder_in_proj(feat_last)
        # (batch_size, c, h, w) -> (num_queries, batch_size, c)
        feat_last = feat_last.flatten(2).permute(2, 0, 1)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
    ...
    def positional_encoding4onnx(self, mask):
        """
        ChongWang for maskformer
        """
        ...
        if  self.positional_encoding.normalize:
            y_embed = (y_embed +  self.positional_encoding.offset) / \
                      (y_embed[:, -1:, :] +  self.positional_encoding.eps) *  self.positional_encoding.scale
            x_embed = (x_embed + self.positional_encoding.offset) / \
                      (x_embed[:, :, -1:] + self.positional_encoding.eps) * self.positional_encoding.scale
        dim_t = torch.arange(
            self.positional_encoding.num_feats, dtype=torch.float32, device=mask.device)
        dim_t = self.positional_encoding.temperature**(2 * (dim_t // 2) / self.positional_encoding.num_feats)
        pos_x = x_embed[:, :, :, None] / dim_t
        pos_y = y_embed[:, :, :, None] / dim_t
        # use `view` instead of `flatten` for dynamically exporting to ONNX
        B, H, W = mask.size()
        pos_x = torch.stack( (pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4)#.view(B, H, W, -1)
        pos_x = pos_x.view(*pos_x.shape[0:3],pos_x.shape[3]*pos_x.shape[4])
        pos_y = torch.stack( (pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), dim=4)#.view(B, H, W, -1)
        pos_y = pos_y.view(*pos_y.shape[0:3],pos_y.shape[3]*pos_y.shape[4])
