...
def ohem_batch_gaussian(score,pos_mask,neg_mask,rate=100,posN=None,mse=500):
    ...
    indices_idxes = np.random.normal(0,scale=mse,size=sampleN)
...
@HEADS.register_module()
class YOLOV3HeadOHEM(YOLOV3Head):
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        loss_cls = self.loss_cls(pred_label, target_label, weight=pos_mask)
        with torch.no_grad():
            pos_and_neg_mask = ohem_batch(pred_conf,pos_mask.squeeze(-1),neg_mask)
...
@HEADS.register_module()
class YOLOV3HeadSample(YOLOV3Head):

    def __init__(self,*args, **kwargs):
        if 'rate' in kwargs:
            self.rate = kwargs['rate']
            kwargs.pop('rate')
        if ('dontsample_kw' in kwargs) is False:
            dontsample_kw=['hahahawoshishenjingbin']
        else:
            dontsample_kw = kwargs['dontsample_kw']
            kwargs.pop('dontsample_kw')
        super(YOLOV3HeadSample, self).__init__(*args,**kwargs)
        if dontsample_kw==None:
            self.dontsample_kw=['hahahawoshishenjingbin']
        else:
            self.dontsample_kw = dontsample_kw
    ...
    def loss(self,
        ...
        for meta in img_metas:
            flag=0
            for word in self.dontsample_kw:
                if word in meta['filename']:
                    flag=1
            dontsample_flags.append(copy.deepcopy(flag))
        dontsample_flags = torch.from_numpy(np.array(dontsample_flags,dtype=int))
        dontsample_flags = dontsample_flags.cuda()

        losses_cls, losses_conf, losses_xy, losses_wh = multi_apply(
            self.loss_single, pred_maps, target_maps_list, neg_maps_list,[dontsample_flags]*3)
    ...
    def loss_single(self, pred_map, target_map, neg_map,dontsample_flags):
        """Compute loss of a single image from a batch.
        ...
        loss_cls = self.loss_cls(pred_label, target_label, weight=pos_mask)
        with torch.no_grad():
            pos_and_neg_mask = sample_batch(pos_mask.squeeze(-1),neg_mask, rate=self.rate)
            for i in range(dontsample_flags.size()[0]):
                if dontsample_flags[i]==1:
                    pos_and_neg_mask[i,...]=1
...
@HEADS.register_module()
class YOLOV3HeadSampleOHEM(YOLOV3Head):
    def __init__(self,*args, **kwargs):
        if ('random_neg_rate' in kwargs) is False:
            self.random_neg_rate=100
        else:
            self.random_neg_rate = kwargs['random_neg_rate']
            kwargs.pop('random_neg_rate')
        if ('OHEM_neg_rate' in kwargs) is False:
            self.OHEM_neg_rate=100
        else:
            self.OHEM_neg_rate = kwargs['OHEM_neg_rate']
            kwargs.pop('OHEM_neg_rate')
        super(YOLOV3HeadSampleOHEM, self).__init__(*args,**kwargs)
    ...
    def loss(self,
        ...
        has_pos = torch.tensor([v.shape[0]>0 for v in gt_labels],dtype=torch.bool,device=device)
        losses_cls, losses_conf, losses_xy, losses_wh = multi_apply(self.loss_single, pred_maps, target_maps_list, neg_maps_list,[has_pos]*3)
    ...
    def loss_single(self, pred_map, target_map, neg_map,has_pos):
        """Compute loss of a single image from a batch.
        ...
        with torch.no_grad():
            ...
            pos_and_neg_mask1 = sample_batch(pos_mask[has_pos].squeeze(-1),neg_mask[has_pos],rate=self.random_neg_rate)
            pos_and_neg_mask2 = ohem_batch(pred_conf[~has_pos], pos_mask[~has_pos].squeeze(-1), neg_mask[~has_pos],rate=self.OHEM_neg_rate)

        loss_conf1 = self.loss_conf( pred_conf[has_pos], target_conf[has_pos], weight=pos_and_neg_mask1)
        loss_conf2 = self.loss_conf( pred_conf[~has_pos], target_conf[~has_pos], weight=pos_and_neg_mask2)
    ...
    def loss_single(self, pred_map, target_map, neg_map,has_pos):
        ...
        with torch.no_grad():
            if self.pos_sample['type']=='OHEM':
                pos_and_neg_mask1 = ohem_batch(pred_conf[has_pos],pos_mask[has_pos].squeeze(-1),neg_mask[has_pos],rate=self.pos_sample['rate'])
            ...
            if self.neg_sample['type']=='OHEM':
                pos_and_neg_mask2 = ohem_batch(pred_conf[~has_pos], pos_mask[~has_pos].squeeze(-1), neg_mask[~has_pos],rate=self.neg_sample['rate'],posN=posN)
                ...
                pos_and_neg_mask2_2 = ohem_batch(pred_conf[~has_pos], pos_mask[~has_pos].squeeze(-1), neg_mask[~has_pos],rate=self.neg_sample['OHEMrate'],posN=posN)
                ...
                pos_and_neg_mask2 = ohem_batch_gaussian(pred_conf[~has_pos], pos_mask[~has_pos].squeeze(-1), neg_mask[~has_pos],
