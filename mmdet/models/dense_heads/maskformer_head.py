...
class MaskFormerHead(AnchorFreeHead):
    ...
    def onnx_export(self, feats, img_metas, **kwargs):
        batch_size = kwargs['batch_size']
        input_img_h, input_img_w = kwargs['input_img_h'],kwargs['input_img_w']
        ...
        # when backbone is swin, memory is output of last stage of swin.
        # when backbone is r50, memory is output of tranformer encoder.
        mask_features, memory = self.pixel_decoder.forward4maskformer(feats, input_img_h, input_img_w)
    ...
    def positional_encoding4onnx(self, mask):
        ...
        if  self.decoder_pe.normalize:
            y_embed = (y_embed +  self.decoder_pe.offset) / \
                      (y_embed[:, -1:, :] +  self.decoder_pe.eps) *  self.decoder_pe.scale
            x_embed = (x_embed + self.decoder_pe.offset) / \
                      (x_embed[:, :, -1:] + self.decoder_pe.eps) * self.decoder_pe.scale
        dim_t = torch.arange(
            self.decoder_pe.num_feats, dtype=torch.float32, device=mask.device)
        dim_t = self.decoder_pe.temperature**(2 * (dim_t // 2) / self.decoder_pe.num_feats)
